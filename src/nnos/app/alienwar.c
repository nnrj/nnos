...
void NNOSMain(void)
{
	...
	win = api_window(winbuf, 336, 261, -1, "Alien War");
	api_boxwin(win, 6, 27, 329, 254, 0);
...
void putstr(int win, char *winbuf, int x, int y, int col, unsigned char *s)
{
	...
	api_boxwin(win + 1, x, y, x + i * 8 - 1, y + 15, 0);
	q = winbuf + y * 336;
	t[1] = 0;
	for (;;) {
		c = *s;
...
void setdec8(char *s, int i){ //将i用十进制表示并存入s
	int j;
	for (j = 7; j >= 0; j--) {
		s[j] = '0' + i % 10;
		i /= 10;
