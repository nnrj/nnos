...
/* void setTimer(TIMER *timer,unsigned int timeout){
	...
	io_cli(); //屏蔽中断，若设定过程中发生中断将造成定时器混乱	
	for(i = 0;i < timerList.number;i++){
		if(timerList.timerx[i]->timeout >= timer->timeout){
			break;
		}
	}
	for(j = timerList.number;j > i;j--){
		timerList.timerx[j] = timerList.timerx[j - 1];
	}
	timerList.number++;
	timerList.timerx[i] = timer;
	timerList.next = timerList.timerx[0]->timeout;
	io_store_eflags(eflags); //恢复现场，恢复之前的中断状态
} */

/* void initTimer(TIMER *timer,FIFO_BUFFER *FIFOBuffer,unsigned char data){
	timer->fifo = FIFOBuffer;
	timer->data = data;
} */

/*定时器设置*/
/* void setTimer(TIMER *timer,unsigned int timeout,FIFO_BUFFER *FIFOBuffer,unsigned char data){
	int eflags;
	eflags = io_load_eflags(); //保护现场，读取标志寄存器，赋值给eflags
	io_cli(); //屏蔽中断，若设定过程中发生中断将造成定时器混乱
...
/*  void setTimer(unsigned int timeout,FIFO_BUFFER *FIFOBuffer,unsigned char data){
	int eflags;
	eflags = io_load_eflags(); //保护现场，读取标志寄存器，赋值给eflags
	io_cli(); //屏蔽中断，若设定过程中发生中断将造成定时器混乱
	timer.timeout = timeout;
...
/*  TIMER *timerAlloc(){
//COUNT i;
if(timerList.number >= TIMER_MAX){
	return NONE; //NONE = 0，未找到
}
	return &timerList.timer[number + 1]; //分配
}
 */
/* TIMER *timerAlloc(){
COUNT i = 0;
for(i = timerList.number;i < TIMER_MAX;i++){
	if(timerList.timer[i].flag == 0){
		return &timerList.timer[i]; //分配
	}
}
...
/* void sortTimerList(){
	...
	TIMER temp;
	for(i = 0;i < timerList.number;i++){
		for(j = i;j < TIMER_MAX;j++){
			if((timerList.timer[i].timeout > timerList.timer[j].timeout) && (timerList.timer[j].timeout != 0)){
				temp = timerList.timer[i];
				timerList.timer[i] = timerList.timer[j];
				timerList.timer[j] = temp;
			}
...
/* void setTimer(TIMER *timer,unsigned int timeout){
	...
	timerList.number++;
	//sortTimerList();
	//timerList.next = timerList.timer[0].timeout;
	if(timerList.next > timer->timeout){
		//timerList.next = timerList.timer[0].timeout;
		//timerList.number--;
		timerList.next = timer->timeout;
	}
...
/* void setTimer(TIMER *timer,unsigned int timeout){
	timer->timeout = (timerList.count + timeout) % TIMER_TIMER_MAX; //定时器相对时间，从当前时间开始多少秒后
	//timer->timeout = timerList.count + timeout; 
	timer->flag = TIMER_FLAG_USING; //设置定时器状态为正在使用(2)
	timerList.number++;
	sortTimerList();
	//timerList.next = timerList.timer[0].timeout;
  	if(timerList.next > timerList.count){
		//timerList.next = &timerList.timer[0];
		//timerList.number--;
		timerList.next = timerList.timer[0].timeout;
	} 
} */
