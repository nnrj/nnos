...
应用程序作者：csdn_mzq 
来源：CSDN 
原文：httpsblog.csdn.netcsdn_mzqarticledetails70859794 
版权声明：本文为博主原创文章，转载请附上博文链接！
-----------------------------------------------------------
移植者：年年软件 移植日期：2019-5-2
===========================================================*/
/*游戏解读：
 某一细胞的邻居包
...
void init()
{
  ...
  for (row = 0; row < MAXROW; row++)
    for (col = 0; col < MAXCOL; col++)
      map[row][col] = DEAD;
  printf("%s","Game of life Program.\n");
  printf("%s","Enter x, y where x, y is living cell");
  printf("0 <= x <= %d, 0 <= y <= %d\n", MAXROW - 1, MAXCOL - 1);
  printf("%s","Terminate with x, y = -1, -1");
  while (1)
  {
    //scanf("%d %d", &row, &col);
	row = 30;col = 30;
