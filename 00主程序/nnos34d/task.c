...
/* TASK *initTaskList(MEMERY_LIST *memeryList){
	...
	load_tr(task->selector); //将任务段选择子装载到tr寄存器
	taskTimer = timerAlloc(); //为任务计时器申请空间
	setTimer(taskTimer,2); //设定任务计时器，0.02秒切换一次
	return task; //将任务指针返回，该默认任务未本任务管理程序
} */
/* TASK *initTaskList(MEMERY_LIST *memeryList){
	COUNT i = 0; //计数器
...
/* void taskSwitch(){
	setTimer(taskTimer,2); //设定任务定时器，时间片尾0.02秒
	if(taskList->number >= 2){
		taskList->now++;
		if(taskList->now == taskList->number){
			taskList->now = 0;
		}
		farjmp(0,taskList->tasks[taskList->now]->selector); //切换到任务
	}
} */
/* void taskSwitch(){
	TASK *task;
	taskList->now++;
	if(taskList->now == taskList->number){ //防止溢出
		taskList->now = 0;
	}
	task = taskList->tasks[taskList->now];
	setTimer(taskTimer,task->priority); //设定任务定时器，时间片设置为响应优先级
	if(taskList->number >=2){ //两个以上任务才进行切换
		farjmp(0,task->selector);
	}
} */

/* void taskRun(TASK *task){
	task->flag = TASK_FLAG_RUNNING; //设置任务状态为正在运行
	taskList->tasks[taskList->number] = task; //将任务挂载到运行队列
	taskList->number++; //运行中的任务数量加1
} */
/* void taskRun(TASK *task,int priority){
	if(priority > 0){ //指定的优先级大于0时改变优先级
		task->priority = priority;
	}
	if(task->flag != 2){
...
/* void taskSleep(TASK *task){ //将处于空闲状态的任务从任务列表中移除
	...
	char taskFlag = 0;
	if(task->flag == 2){
		if(task == taskList->tasks[taskList->now]){
			taskFlag = 1; //若为自休眠，则需要进行任务切换
		}
	}
	for(i = 0;i < taskList->number;i++){ //遍历任务列表
		if(taskList->tasks[i] == task){ //找需要休眠的任务
			break;
		}
	}
	taskList->number--;
	if(i < taskList->now){ //休眠的任务在当前运行任务之前，移动任务项
		taskList->now--; 
	}
	for(;i < taskList->number;i++){
		taskList->tasks[i] = taskList->tasks[i+1];
	}
	task->flag = TASK_FLAG_USED; //转入就绪状态
	if(taskFlag != 0){
		if(taskList->now >= taskList->number){ //now值不能大于number
			taskList->now = 0;
		}
		farjmp(0,taskList->tasks[taskList->now]->selector);
	}
} */

/* void initTask(void)
{
	taskTimer = timerAlloc();
	setTimer(taskTimer, 2);
	taskTR = 3 * 8;
	return;
}
